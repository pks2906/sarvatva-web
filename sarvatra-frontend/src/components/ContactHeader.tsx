...
const ContactHeader = () => {
  ...
  return (
    <div className={`font-cormorant fixed flex px-4 items-center justify-between top-0 left-0 w-full z-20 transition-colors duration-300 ${scrolled ? 'bg-[#131313] backdrop-blur-sm shadow-lg' : ''}`}>
      <Link to={'/'}>
      ...
      <div className={`hidden lg:flex space-x-2 md:gap-x-4 text-sm md:text-md`}>
        ...
        <Link to={'/about'}>
          <p className={`py-4 md:px-4 text-[18px] font-semibold rounded transition-all ${scrolled ? 'hover:text-[#EDE6D6] text-[#EDE6D6]/80' : 'text-[#131313] hover:bg-black hover:bg-opacity-30 hover:text-[#EDE6D6]'}`}>The Craft of Wholeness</p>
        </Link>
        <Link to={'/contact'}>
          <p className={`py-4 md:px-4 text-[18px] font-semibold rounded transition-all ${scrolled ? 'hover:text-[#EDE6D6] text-[#EDE6D6]/80' : 'text-[#131313] hover:bg-black hover:bg-opacity-30 hover:text-[#EDE6D6]'}`}>Let's Connect</p>
        </Link>
      </div>
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            ...
            <Link to={'/contact'} className='w-full'>
              <p className={`py-4 text-[18px] transition-all text-[#EDE6D6]/80 border-b-[0.5px] border-[#EDE6D6]/10 px-6`}>Let's Connect</p>
            </Link>
          </motion.div>
        )}
      </AnimatePresence>
